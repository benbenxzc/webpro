      ...
      <%
       ...
       <div class="row">       
            ...
            <div class="span8">
	          ...
	          <li class="active">
	            <a href="PersonalPage.jsp"  style="font-family:微软雅黑">我参与的活动</a>
	          </li>
	          <li><a href="MyActivities.jsp"  style="font-family:微软雅黑">我发起的活动</a></li>
	        </ul>
	        <table class="table">
          <tbody>
	        <%
	        	Vector<Integer> activityId = attendbean.getattend(userId);
	        	...
	        	{	        	
		        	...
		        	{
		        		j = 0;
		        		Vector<String> activityInfo = attendbean.getActivityInfoByIdMyActivities(activityId.get(k));
		        		int id = userbean.getUserId(activityInfo.get(j));
          ...
          <td>          
            ...
            <%=activityInfo.get(j)%> 				<!-- username -->
             <%j++;%>            <!-- activityId -->
            : <a href='ActivityInformation.jsp?activityId=<%=activityInfo.get(j)%>'  >  
            <%j++;%><%=activityInfo.get(j) %></a>				<!-- activityname -->
           <%j++;%>          <!-- address -->			           
            </h4>
           
            <div class="content">            
                    <div class="status-lbs" data-wiki="{}">
                            <span class="location"   style="font-family:微软雅黑">地址：<a><%=activityInfo.get(j)%><%j++;%></a> </span><br>
                            <% if(activityInfo.get(j).equals("1")){ %>
					            <h5 style="float:right;color:blue;font-weight:normal;font-family:微软雅黑"><i class="icon-ok"></i> 活动已结束！</h5>
					            <%} %>	
					            <%j++; %>
                            <span class="location"  style="font-family:微软雅黑">分类：<a><%=activityInfo.get(j)%><%j++;%></a> </span>
                    </div>
            </div>  
            <div class="content">
            <div class="row">          
            ...
            <div class="span1"></div>  
                    ...
                    <%	
                    		...
                    		if(!attendedUser.isEmpty())
                    		{
	                    		...
	                    		{                    			
	                    			int userid = attendedUser.get(m);
	                    			String name = userbean.getUsername(userid);		//attendedUser为userId的容器，转换为username                  
                     %>
                            
                            <a href="OthersPersonalPage.jsp?userId=<%=userid%>"><%=name%></a>&nbsp;&nbsp;
                            <%
                            	}
                            }    
                            else
            ...
            <div class="span1"></div>
                    <div class="status-lbs" data-wiki="{}">                    
                            <span class="location"   style="font-family:微软雅黑">&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;开展时间：<%=activityInfo.get(j)%><%j++;%> </span>&nbsp;&nbsp;
                    </div>
            </div>        
            </div>                        
            </article>
